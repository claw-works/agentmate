...
**K3.1 / K3.2 / K3.3 / K3.4 / K3.5 已实现并在真实环境验证**，详见
`knowledge-wiki-compiler-k3-v0.1.md` §13–§15。剩余 K3.6–K3.9 仍为设计。
...
## 2026-07-30 K3.4 增量编译已实现

新增 `internal/knowledge/wiki_incremental.go`、`wiki_incremental_repo.go`；check 新增两条
不变量；11 个集成/单元测试。无新增 migration —— `pages_reused`、`derived_from_build_id`、
`parent_build_id` 在 000027 就已建模，这次只是第一次真正去写它们。

**为什么必须做**：全量编译把整个 wiki 塞进一次模型回复，语料上限是硬的 —— 输出预算已经
从 4096 抬到 16384 再到 32768，撞过两次天花板，没有下一档可调。增量不是省钱优化，是唯一出路。

机制：diff raw 源 → 算影响面闭包 → 只把受影响的页给模型 → 其余从 parent 拷过来。
**影响面由数据库算出，不问模型**：低估会让 wiki 留下引用已变更文档的过期断言，而下游没有任何
东西能发现。闭包是两跳——引用了变更/删除文档的页（直接过期），加一跳入链者（不是它过期，
而是重编可能删掉或改名它的目标，复用页指向不存在的页就是悬空链、被 check 拒；把入链者一起
拉进来，等于让删页的那次编译同时负责修好指向它的人）。**刻意不做传递闭包**——任何连接良好的
知识库上全闭包都会收敛到整个 wiki，那是披着增量外衣的全量重建。

实测（`platform/retrieval`，4 篇文档改 1 篇）：输出 token **8707 → 4841（降 44%）**，
5 页中 3 页复用。输入基本没动（2618 → 2712）——4 篇文档的语料下，"变更文档 + 受影响页正文
+ 全部页路径清单"跟"直接发全部文档"体量相当。**输入的节省随语料规模才显现，这个规模下没有。**
这没问题：增量存在的理由是输出天花板，而那个数字动了。

### 评审抓出的五个缺陷（全部已修，评审结论是"不应合入"）

用 `code-reviewer` 子 agent 审时抓出五条，每条都是**结构 check 全过而 wiki 悄悄是错的**——
正是复用会引入的那一类缺陷。五条全部成立，其中第 2 条我在真实环境里亲眼见过还合理化过去了。

1. **受影响页漏回时静默保留旧页**。原逻辑把"排进重写但模型没返回"的页原样恢复，注释理由是
   "过期文本好过图上有洞"。这条不成立：citation 路径仍然存在，所以每条结构规则都过，而页面
   断言的是它的源已经不再支持的东西，agent 读了分辨不出来。改为 check 新规则
   `incremental_coverage` 直接判失败——"半个 wiki 比没有更糟"的同一逻辑，套用在"半更新的 wiki"上。
2. **空 diff 快路径会跳过本该发生的重编**。parent 由另一个模型/prompt/compiler/profile 产出时，
   raw diff 是空的而每一页其实都需要重写；原逻辑复用全部页、零模型调用，然后把本次 build 的
   provenance 盖在那个模型从未产出过的文本上。**真实环境里就发生过**：build `a90d6393` 记录
   `model=qwen3.7-plus`，而 5 个内容页全部来自 deepseek 的 build。新增 `ErrIncompatibleParent`，
   在 enqueue 与 worker 两处拒绝，并明说该用 full。
3. **模型可以改/删计划外的页**。它拿到了全部页路径（为了能自由链接），因此可以返回一个没人
   要求它碰的页覆盖复用页；结构上一个被覆盖的页仍然是合法页，check 不会发现。现在越权写入/
   删除被就地丢弃并记 `page_rejected`，同时 check 的 `incremental_scope` 判失败——丢弃保住了
   计划承诺保住的页，判失败是因为编译器无视范围本身是值得暴露的缺陷。这条也让
   `ScheduledPaths` 从"记录"变成真正的约束：不约束的记录不算记录。
4. **复用页的 citation 仍指向 parent revision 的 document 行**。document 是按 revision 存的，
   拷贝 citation 时只清了 ID/BuildID/PageID 没清 `document_id`，于是 build 声称基于新 revision
   而 citation 落在旧 revision，而 check 只校验 path 所以看不见。现在按当前 revision 的 path
   重解析；解析不到就置空，交给 `citation_resolvable` 报出来。
5. **模型缺少正确重写所需的上下文**。原 prompt 只给变更文档正文和受影响页的 path/kind/title/
   content，不给该页现有的 citation 与 typed link，也不给它仍引用的未变更文档。一个同时引用
   变更 A 与未变更 B 的页被要求整体重写时，模型无法知道哪些 claim 来自 B，唯一安全动作就是
   丢掉它交代不了的部分——有效事实和图边静默丢失。现在补齐三者，且**上下文文档按影响面而非
   语料收集**，不会把增量存在的理由抵消掉。

真实环境确认：影响面闭包正确（改 `raw/embedding.md` → scheduled 3 页、recompiled 3 页、
reused 2 页，保留页未混入、三个清单互斥）；跨编译器身份的增量返回 400 并说清 parent 用的
