...
完整目标架构见 [Skill + Knowledge Architecture v0.3](skill-knowledge-architecture-v0.1.md)。
...
- 不把任意文档目录都解释为 Skill。
- 不把 Skill Registry 用作通用领域知识库：领域文档、事实语料和持久化 wiki 属于未来独立的 Knowledge Registry（见 [Skill + Knowledge Architecture v0.3](skill-knowledge-architecture-v0.1.md)）。
...
L2 resource manifest 的语义是 **Skill execution assets**，不是通用领域知识。领域文档和事实语料属于未来独立 Knowledge Registry 的 K0/K1/K2 披露轴；过渡期内既有 reference 文件仍可通过现有 API 加载，但不应再向 Skill package 中新增独立知识语料（见 [Skill + Knowledge Architecture v0.3](skill-knowledge-architecture-v0.1.md)）。
...
独立于 Skill Registry 的 Knowledge 域按 [Skill + Knowledge Architecture v0.3](skill-knowledge-architecture-v0.1.md) 的 K1–K5 路线推进（K1–K5 是实施里程碑编号，与知识披露层级 K0–K3 无关）：
