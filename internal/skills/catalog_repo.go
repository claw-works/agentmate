...
func (r *Repo) UpsertCompiledCatalog(ctx context.Context, artifact CompiledSkillCatalog) (*CompiledSkillCatalog, error) {
	...
	err = r.pool.QueryRow(ctx,
		`INSERT INTO skill_compiled_catalogs
		 (account_id, skill_version_id, skill_name, compiler_name, compiler_version, input_package_hash,
		  description, triggers, capabilities, constraints, dependencies, resource_manifest, compiled_at)
		 SELECT $1, version.id, $3, $4, $5, $6, $7, $8::jsonb, $9::jsonb, $10::jsonb, $11::jsonb, $12::jsonb, $13
		...
		capabilities, constraints, dependencies, manifest, artifact.CompiledAt,
	).Scan(
		...
		&rawTriggers, &rawCapabilities, &rawConstraints, &rawDependencies, &rawManifest, &stored.CompiledAt,
	)
	if err != nil {
		return nil, err
	}
	...
	if err := decodeCompiledJSON(&stored, rawTriggers, rawCapabilities, rawConstraints, rawDependencies, rawManifest); err != nil {
		return nil, err
	}
	return &stored, nil
}
...
func (r *Repo) ListActiveCatalog(ctx context.Context, accountID string, params SkillCatalogListParams) ([]catalogRecord, error) {
	...
	rows, err := r.pool.Query(ctx,
		`SELECT `+catalogVersionColumns+`,
		        ...
		        catalog.description, catalog.triggers, catalog.capabilities, catalog.constraints,
		        catalog.dependencies, catalog.resource_manifest, catalog.compiled_at
		 FROM skill_versions AS version
	...
	for rows.Next() {
		...
		destinations := append(versionDestinations,
			...
			&rawTriggers, &rawCapabilities, &rawConstraints, &rawDependencies, &rawManifest, &compiledAt,
		)
...
func decodeCompiledJSON(artifact *CompiledSkillCatalog, rawTriggers, rawCapabilities, rawConstraints, rawDependencies, rawManifest []byte) error {
